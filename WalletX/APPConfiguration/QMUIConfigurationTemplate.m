...
#pragma mark - Global Color
    ...
    QMUICMI.separatorColor = [UIColorMakeWithHex(@"#808A9D") qmui_colorWithAlphaAddedToWhite:0.1];                      // UIColorSeparator : 全局默认的分割线颜色，默认用于列表分隔线颜色、UIView (QMUIBorder) 分隔线颜色
    QMUICMI.separatorDashedColor = UIColorMake(17, 17, 17);                     // UIColorSeparatorDashed : 全局默认的虚线分隔线的颜色，默认 QMUIKit 暂时没用到
    QMUICMI.placeholderColor = [UIColorMakeWithHex(@"#808A9D") qmui_colorWithAlphaAddedToWhite:0.5];                                    // UIColorPlaceholder，全局的输入框的 placeholder 颜色，默认用于 QMUITextField、QMUITextView，不影响系统 UIKit 的输入框
...
#pragma mark - UIButton
    ...
    QMUICMI.buttonTintColor = UIColor.tintColor;                              // ButtonTintColor : QMUIButton 默认的 tintColor，不影响系统的 UIButton
    
    
#pragma mark - TextInput
    QMUICMI.textFieldTextColor = UIColorMakeWithHex(@"#333333");                     // TextFieldTextColor : QMUITextField、QMUITextView 的 textColor，不影响 UIKit 的输入框
    QMUICMI.textFieldTintColor = UIColor.tintColor;                          // TextFieldTintColor : QMUITextField、QMUITextView 的 tintColor，不影响 UIKit 的输入框
...
#pragma mark - UISwitch
    QMUICMI.switchOnTintColor = UIColor.tintColor;                           // SwitchOnTintColor : UISwitch 打开时的背景色（除了圆点外的其他颜色）
    QMUICMI.switchOffTintColor =  UIColor.separatorColor;                    // SwitchOffTintColor : UISwitch 关闭时的背景色（除了圆点外的其他颜色）
...
#pragma mark - NavigationBar
    ...
    QMUICMI.navBarBackgroundImage = [UIImage new];    // NavBarBackgroundImage : UINavigationBar 的背景图，注意 navigationBar 的高度会受多个因素（是否全面屏、是否使用了 navigationItem.prompt、是否将 UISearchBar 作为 titleView）的影响，要检查各种情况是否都显示正常。
    if (@available(iOS 15.0, *)) {
    ...
    QMUICMI.navBarTintColor = UIColor.blackColor;                                     // NavBarTintColor : NavBarContainerClasses 里的 UINavigationBar 的 tintColor，也即导航栏上面的按钮颜色
    QMUICMI.navBarTitleColor = UIColor.blackColor;                                 // NavBarTitleColor : UINavigationBar 的标题颜色，以及 QMUINavigationTitleView 的默认文字颜色
    ...
    QMUICMI.navBarBackIndicatorImage = [UIImage imageNamed:@"navigation_back_button"];                                     // NavBarBackIndicatorImage : 导航栏的返回按钮的图片，图片尺寸建议为(13, 21)，否则最终的图片位置无法与系统原生的位置保持一致
    
    QMUICMI.navBarCloseButtonImage = [UIImage imageNamed:@"navigation_close_button"];     // NavBarCloseButtonImage : QMUINavigationButton 用到的 × 的按钮图片
    ...
    QMUICMI.navBarAccessoryViewTypeDisclosureIndicatorImage = [UIImage imageNamed:@"navigation_right_button"];;     // NavBarAccessoryViewTypeDisclosureIndicatorImage : QMUINavigationTitleView 右边箭头的图片
...
#pragma mark - TabBar
    ...
    QMUICMI.tabBarItemTitleColor = UIColorMakeWithHex(@"#999999");             // TabBarItemTitleColor : 未选中的 UITabBarItem 的标题颜色
    QMUICMI.tabBarItemTitleColorSelected = UIColorMakeWithHex(@"#4162FF");                // TabBarItemTitleColorSelected : 选中的 UITabBarItem 的标题颜色
    QMUICMI.tabBarItemImageColor = UIColorMakeWithHex(@"#999999");;                        // TabBarItemImageColor : UITabBarItem 未选中时的图片颜色（该配置项在 iOS 12 及以下的系统对 QMUIThemeImage 无效，请自行在 provider 内处理颜色。注意非选中状态的图片需要指定为 UIImageRenderingModeAlwaysOriginal，系统限制如此）
    QMUICMI.tabBarItemImageColorSelected = UIColorMakeWithHex(@"#4162FF");        // TabBarItemImageColorSelected : UITabBarItem 选中时的图片颜色
...
#pragma mark - Toolbar
    ...
    QMUICMI.toolBarTintColor = UIColor.qmui_systemTintColor;                            // ToolBarTintColor : NavBarContainerClasses 里的 UIToolbar 的 tintColor，以及 QMUIToolbarButton normal 状态下的文字颜色
...
#pragma mark - SearchBar
    ...
    QMUICMI.searchBarTextFieldBackgroundImage = [UIImage new];       // SearchBarTextFieldBackgroundImage : QMUISearchBar 里的文本框的背景图，图片高度会决定输入框的高度
    ...
    QMUICMI.searchBarBackgroundImage = [UIImage new];     // SearchBarBackgroundImage : 搜索框的背景图，如果需要设置底部分隔线的颜色也请绘制到图片里
    QMUICMI.searchBarTintColor = UIColor.qmui_systemTintColor;                          // SearchBarTintColor : QMUISearchBar 的 tintColor，也即上面的操作控件的主题色
    QMUICMI.searchBarTextColor = UIColor.lightTextColor;                                  // SearchBarTextColor : QMUISearchBar 里的文本框的文字颜色
...
#pragma mark - Plain TableView
    ...
    QMUICMI.tableViewCellTitleLabelColor = UIColor.lightTextColor;             // TableViewCellTitleLabelColor : QMUITableViewCell 的 textLabel 的文字颜色
    QMUICMI.tableViewCellDetailLabelColor = UIColor.lightTextColor;     // TableViewCellDetailLabelColor : QMUITableViewCell 的 detailTextLabel 的文字颜色
    QMUICMI.tableViewCellBackgroundColor = UIColorForBackground;                 // TableViewCellBackgroundColor : QMUITableViewCell 的背景色
    QMUICMI.tableViewCellSelectedBackgroundColor = [UIColor.lightTextColor qmui_colorWithAlphaAddedToWhite:0.6];  // TableViewCellSelectedBackgroundColor : QMUITableViewCell 点击时的背景色
    ...
    QMUICMI.tableViewCellDisclosureIndicatorImage = [UIImage new];    // TableViewCellDisclosureIndicatorImage : QMUITableViewCell 当 accessoryType 为 UITableViewCellAccessoryDisclosureIndicator 时的箭头的图片
    QMUICMI.tableViewCellCheckmarkImage = [UIImage new];  // TableViewCellCheckmarkImage : QMUITableViewCell 当 accessoryType 为 UITableViewCellAccessoryCheckmark 时的打钩的图片
    QMUICMI.tableViewCellDetailButtonImage = [UIImage new]; // TableViewCellDetailButtonImage : QMUITableViewCell 当 accessoryType 为 UITableViewCellAccessoryDetailButton 或 UITableViewCellAccessoryDetailDisclosureButton 时右边的 i 按钮图片
    ...
    QMUICMI.tableViewSectionHeaderBackgroundColor = UIColor.separatorColor;                         // TableViewSectionHeaderBackgroundColor : Plain 类型的 QMUITableView sectionHeader 的背景色
    QMUICMI.tableViewSectionFooterBackgroundColor = UIColor.secondarySystemFillColor;                         // TableViewSectionFooterBackgroundColor : Plain 类型的 QMUITableView sectionFooter 的背景色
    ...
    QMUICMI.tableViewSectionHeaderTextColor = [UIColor grayColor];                                             // TableViewSectionHeaderTextColor : Plain 类型的 QMUITableView sectionHeader 里的文字颜色
...
#pragma mark - Grouped TableView
    ...
    QMUICMI.tableViewGroupedCellBackgroundColor = UIColor.lightTextColor;                    // TableViewGroupedCellBackgroundColor : Grouped 类型的 QMUITableView cell 背景色
...
#pragma mark - Others
    ...
    QMUICMI.defaultStatusBarStyle = UIStatusBarStyleDarkContent;               // DefaultStatusBarStyle : 默认的状态栏样式，默认值为 UIStatusBarStyleDefault，也即在 iOS 12 及以前是黑色文字，iOS 13 及以后会自动根据当前 App 是否处于 Dark Mode 切换颜色。如果你希望固定为白色，请设置为 UIStatusBarStyleLightContent，固定黑色则设置为 UIStatusBarStyleDarkContent。
...
- (BOOL)shouldApplyTemplateAutomatically {
    ...
    NSString *selectedThemeIdentifier = [[NSUserDefaults standardUserDefaults] stringForKey:@"QDThemeIdentifierDefault"];
...
- (UIColor *)themeTintColor {
    return UIColorMakeWithHex(@"#4162FF");
}

- (UIColor *)themeTitleTextColor {
    return UIColorMakeWithHex(@"#333333");
}
...
- (UIColor *)themeDescriptionTextColor {
    return UIColorMakeWithHex(@"#808A9D");
}

- (UIColor *)themePlaceholderColor {
    return [UIColorMakeWithHex(@"#808A9D") qmui_colorWithAlphaAddedToWhite:0.5];
}
